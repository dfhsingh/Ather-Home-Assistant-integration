...
class AtherAPI:
    ...
    async def generate_otp(self, phone_number: str) -> bool:
        ...
        try:
            async with self._session.post(
                GENERATE_OTP_URL, json=payload, headers=COMMON_HEADERS
            ) as resp:
                if resp.status == 200:
                    return True
    ...
    async def verify_otp(self, phone_number: str, otp: str) -> dict | None:
        ...
        try:
            async with self._session.post(
                VERIFY_OTP_URL, json=payload, headers=COMMON_HEADERS
            ) as resp:
                if resp.status == 200:
    ...
    async def get_user_id(self, token: str) -> str | None:
        ...
        try:
            async with self._session.get(ME_URL, headers=headers) as resp:
                if resp.status == 200:
                    data = await resp.json()
