...
from .helpers import is_binary_value
...
async def async_setup_entry(
    ...
    entities = [
        ...
        # TPMS Sensors
        AtherTpmsSensor(coordinator, "front"),
        AtherTpmsSensor(coordinator, "rear"),
...
class AtherSensor(AtherEntity, SensorEntity):
    """Base class for Ather sensors."""

    # Logic moved to AtherEntity in entity.py
    pass
...
class AtherTpmsSensor(AtherSensor):
    """Representation of TPMS Tyre Pressure."""

    # Removed device_class so HA doesn't auto-convert psi to kPa
    _attr_native_unit_of_measurement = "psi"
    _attr_suggested_unit_of_measurement = "psi"
    _attr_suggested_display_precision = 1
    _attr_state_class = SensorStateClass.MEASUREMENT
    _attr_icon = "mdi:tire"

    def __init__(self, coordinator, position: str) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self.position = position  # "front" or "rear"
        self._attr_name = f"{position.capitalize()} Tyre Pressure"

    @property
    def unique_id(self) -> str:
        return f"ather_{self.coordinator.scooter_id}_tpms_{self.position}"

    @property
    def native_value(self) -> float | None:
        """Return the state of the sensor."""
        tpms_data = self.coordinator.get_data("tpms", {})
        key = f"{self.position}TyrePressure"
        val = tpms_data.get(key)
        if val is not None:
            return float(val)
        return None
    ...
    def extra_state_attributes(self) -> dict[str, any]:
        ...
        uuid_key = (
            "backTyreUUID" if self.position == "rear" else f"{self.position}TyreUUID"
        )

        attrs.update(
            {
                "tyre_uuid": tpms_data.get(uuid_key),
                "battery_voltage": tpms_data.get(f"{self.position}BatteryVoltage"),
                "high_pressure_flag": tpms_data.get(f"{self.position}HighPressureFlag"),
                "low_pressure_flag": tpms_data.get(f"{self.position}LowPressureFlag"),
                "safety_hazard_flag": tpms_data.get(f"{self.position}SafetyHazardFlag"),
            }
        )
        return attrs


class AtherTpmsBatterySensor(AtherSensor):
    """Representation of TPMS Battery Level."""
    ...
    def __init__(self, coordinator, position: str) -> None:
        ...
        super().__init__(coordinator)
        self.position = position  # "front" or "rear"
        self._attr_name = f"{position.capitalize()} TPMS Battery"

    @property
    def unique_id(self) -> str:
        return f"ather_{self.coordinator.scooter_id}_tpms_{self.position}_battery"

    @property
    def native_value(self) -> int | None:
        """Return the state of the sensor."""
        tpms_data = self.coordinator.get_data("tpms", {})
        key = f"{self.position}BatteryVoltage"
        val = tpms_data.get(key)
        if val is not None:
            return int(val)
        return None
