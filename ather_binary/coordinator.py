...
class AtherCoordinator:
    ...
    async def async_request_refresh(self):
        ...
        # This coordinator is primarily push-based (WebSocket).
        # However, we can trigger a ping to solicit an update if needed.
        # For now, we'll mostly rely on the WS stream, but we MUST implement this
        # to satisfy the CoordinatorEntity interface and prevent errors.
        if self._shutdown:
            return

        # We could call self.async_ping_scooter() here, but that has rate limits.
        # Just notify listeners to ensure state is consistent.
    ...
    def _process_data(self, data: Any, path: str = None):
        ...
        if current_state != "riding" and self._previous_state_for_rides == "riding":
            ...
            if self.ride_manager:
                # Add 1800s delay (30 mins) to allow server to process/index the ride
                _LOGGER.info(
                    "Scheduling Post-Ride Sync in 1800 seconds (30 mins) to allow server processing."
                )
